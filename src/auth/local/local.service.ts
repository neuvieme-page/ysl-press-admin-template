...
export class AuthLocalService {
  ...
  async signIn({
    email,
    password,
  }: LocalSigninDTO): Promise<BearerResponse | null> {
    ...
    if (!identity)
      throw new UnauthorizedException(
        'Cannot signin, there is no user registered',
      )
    ...
    return {
      access_token: this.jwtService.sign(payload, {
        expiresIn: configService.isProduction() ? '1d' : '365d',
      }),
    }
  }
  ...
  async signUp({
    ...
    password,
    email,
  }: LocalSignupDTO): Promise<BearerResponse | null> {
